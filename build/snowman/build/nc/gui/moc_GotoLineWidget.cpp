/****************************************************************************
** Meta object code from reading C++ file 'GotoLineWidget.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../src/nc/gui/GotoLineWidget.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'GotoLineWidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_nc__gui__GotoLineWidget_t {
    QByteArrayData data[8];
    char stringdata[91];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_nc__gui__GotoLineWidget_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_nc__gui__GotoLineWidget_t qt_meta_stringdata_nc__gui__GotoLineWidget = {
    {
QT_MOC_LITERAL(0, 0, 23),
QT_MOC_LITERAL(1, 24, 8),
QT_MOC_LITERAL(2, 33, 0),
QT_MOC_LITERAL(3, 34, 10),
QT_MOC_LITERAL(4, 45, 2),
QT_MOC_LITERAL(5, 48, 14),
QT_MOC_LITERAL(6, 63, 7),
QT_MOC_LITERAL(7, 71, 18)
    },
    "nc::gui::GotoLineWidget\0activate\0\0"
    "deactivate\0go\0indicateStatus\0success\0"
    "rememberCompletion\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_nc__gui__GotoLineWidget[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x0a,
       3,    0,   45,    2, 0x0a,
       4,    0,   46,    2, 0x08,
       5,    1,   47,    2, 0x08,
       5,    0,   50,    2, 0x28,
       7,    0,   51,    2, 0x08,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Bool,    6,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void nc::gui::GotoLineWidget::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        GotoLineWidget *_t = static_cast<GotoLineWidget *>(_o);
        switch (_id) {
        case 0: _t->activate(); break;
        case 1: _t->deactivate(); break;
        case 2: _t->go(); break;
        case 3: _t->indicateStatus((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 4: _t->indicateStatus(); break;
        case 5: _t->rememberCompletion(); break;
        default: ;
        }
    }
}

const QMetaObject nc::gui::GotoLineWidget::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_nc__gui__GotoLineWidget.data,
      qt_meta_data_nc__gui__GotoLineWidget,  qt_static_metacall, 0, 0}
};


const QMetaObject *nc::gui::GotoLineWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *nc::gui::GotoLineWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_nc__gui__GotoLineWidget.stringdata))
        return static_cast<void*>(const_cast< GotoLineWidget*>(this));
    return QWidget::qt_metacast(_clname);
}

int nc::gui::GotoLineWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
